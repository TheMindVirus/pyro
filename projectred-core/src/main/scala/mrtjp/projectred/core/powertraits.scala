...
class PowerConductor(val parent:IPowerConnectable, ids:Seq[Int])
{
    ...
    def applyCurrent(I:Double): AnyVal = 
    {
        voltage()
        Vflow += I
        Iflow += math.abs(I)
    ...
    def applyPower(P:Double): AnyVal = 
    {
    ...
    def surge(cond:PowerConductor, id:Int): AnyVal =
    {
        if (cond == null) false
    ...
    def applySurge(from:PowerConductor, Iin:Double): AnyVal = 
    {
        surgeIn += from
        applyCurrent(Iin)
    }

    def save(tag:CompoundNBT): AnyVal = 
    {
        for (i <- flows.indices)
    ...
    def load(tag:CompoundNBT): AnyVal = 
    {
        for (i <- flows.indices)
...
trait TPowerDrawPoint extends PowerConductor
{
    ...
    abstract override def update(): AnyVal = 
    {
        super.update()
    ...
    abstract override def save(tag:CompoundNBT): AnyVal = 
    {
        super.save(tag)
    ...
    abstract override def load(tag:CompoundNBT): AnyVal = 
    {
        super.load(tag)
