...
object WirePropagator
{
    ...
    def setRedwiresConnectable(b:Boolean): AnyVal = {rwConnectable.set(b)}
    ...
    def addNeighborChange(pos:BlockPos): AnyVal = 
    {
        currentRun.neighborChanges += pos
    }

    def addPartChange(part:TMultiPart): AnyVal = 
    {
        currentRun.partChanges.put(part.tile, part)
    }

    def logCalculation(): AnyVal = 
    {
        if (finishing != null) finishing.recalcs += 1
    }

    def propagateTo(part:IWirePart, from:TMultiPart, mode:Int): AnyVal = 
    {
        var p = currentRun
    ...
    def propagateTo(part:IWirePart, mode:Int): AnyVal = 
    {
        propagateTo(part, notApart, mode)
    }

    def propagateAnalogDrop(part:IWirePart): AnyVal = 
    {
...
//        if (CommandDebug.WIRE_READING) println(recalcs+" recalculations")
    ...
    def start(parent:PropagationRun, world:World): AnyVal = 
    {
        this.world = world
        this.parent = parent
...
/*
        do {
            ...
            pChange = true //we emptied it, probably changed it, but if we didnt, the loop will break anyway.
            ptmp.foreach(_.go())
...
*/
    ...
    def add(part:IWirePart, from: TMultiPart, mode:Int): AnyVal = 
    {
        if (from != lastCaller) {
            lastCaller = from
            count += 1
    ...
    def addAnalogDrop(part:IWirePart): AnyVal = 
    {
