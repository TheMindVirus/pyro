...
trait TPropagationCommons extends TMultiPart with IWirePart
{
    ...
    def propagate(prev:TMultiPart, mode:Int): AnyVal

    def propagateOther(mode:Int): AnyVal = {}

    def propagateExternal(to:TMultiPart, at:BlockPos, from:TMultiPart, mode:Int): AnyVal
    {
        if (to != null) {
            if (to == from) return
    ...
    def propagateInternal(to:TMultiPart, from:TMultiPart, mode:Int): AnyVal = 
    {
        if (to == from) return
        propagateTo(to, mode)
    }
...
trait TFacePropagation extends TPropagationCommons with TFaceConnectable
{
    ...
    override def propagate(prev:TMultiPart, mode:Int): AnyVal = 
    {
...
trait TRSPropagationCommons extends TPropagationCommons
{
    ...
    def getSignal:Int
    def setSignal(signal:Int): AnyVal

    override def updateAndPropagate(prev:TMultiPart, mode:Int): AnyVal = 
    {
