...
//    val guiPacket = 3
    ...
    private[handler] def handleTilePacket(world:World, packet:PacketCustom, pos:BlockPos): AnyVal = 
    {
    ...
    def sendTilePacket(world:World, pos:BlockPos, packet:PacketCustom): AnyVal = 
    {
...
{
    override def handlePacket(packet: PacketCustom, mc: Minecraft, handler: IClientPlayNetHandler): AnyVal = 
    {
        val world = mc.level
...
{
    override def handlePacket(packet: PacketCustom, sender: ServerPlayerEntity, handler: IServerPlayNetHandler): AnyVal = 
    {
        packet.getType match
        {
