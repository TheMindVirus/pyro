...
class ClipNode extends TNode
{
    ...
    override protected[gui] def drawBack(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = 
    {
        if (!hidden)
        {
    ...
    override protected[gui] def drawFront(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = 
    {
        if (!hidden)
        {
            val dp = mouse-position
    ...
    private def onChildPredraw(): AnyVal = 
    {
    ...
    private def onChildPostdraw(): AnyVal = 
    {
...
object ClipNode
{
    def tempDisableScissoring(): AnyVal = 
    {
        GL11.glPushAttrib(GL11.GL_SCISSOR_BIT)
        GL11.glDisable(GL11.GL_SCISSOR_TEST)
    }

    def tempEnableScissoring(): AnyVal = 
    {
        GL11.glPopAttrib()
    }
}
