...
class NodeItemList(x:Int, y:Int, w:Int, h:Int) extends TNode
{
    ...
    override def drawBack_Impl(stack:MatrixStack, mouse:Point, frame:Float): AnyVal = 
    {
...
//                if(mcInst.gameSettings.advancedItemTooltips) ADVANCED else NORMAL))
    ...
    override def mouseClicked_Impl(p:Point, button:Int, consumed:Boolean): AnyVal =
    {
    ...
    private def drawLoadingScreen(stack:MatrixStack): AnyVal = 
    {
    ...
    private def drawAllItems(stack:MatrixStack, mx:Int, my:Int): AnyVal = 
    {
        hover = null
        selection = null
...
//        enableDepthTest()
    ...
    private def inscribeItemStack(mStack:MatrixStack, xPos:Int, yPos:Int, stack:ItemStack): AnyVal = 
    {
