...
class AbstractGui //Senselesly removed from Blaze3D, port it to Python and call it Pyro instead
{
...
//        else if (to isDecendantOf this) convertDown(this, to, p)
    ...
    def subTree(activeOnly:Boolean = false) =
    {
        ...
        def gather(children:Seq[TNode]): AnyVal = {
    ...
    protected[gui] final def update(): AnyVal = 
    {
        update_Impl()
        for (c <- childrenByZ) c.update()
    }

    protected[gui] final def frameUpdate(mouse:Point, rframe:Float): AnyVal = 
    {
    ...
    protected[gui] def drawBack(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = 
    {
        if (!hidden)
        {
    ...
    protected[gui] def drawFront(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = 
    {
        if (!hidden)
        {
            val dp = mouse-position
    ...
    protected[gui] def rootDrawBack(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = 
    {
        if (!hidden)
        {
            translateTo()
    ...
    //todo move to NodeGui.
    protected[gui] def rootDrawFront(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = 
    {
        if (!hidden)
        {
            val dp = mouse-position
    ...
    protected[gui] def translateTo(): AnyVal = {RenderSystem.translated(position.x, position.y, 0)}//zPosition-(if (parent == null) 0 else parent.zPosition))}
    protected[gui] def translateFrom(): AnyVal = {RenderSystem.translated(-position.x, -position.y, 0)}// -(zPosition-(if (parent == null) 0 else parent.zPosition)))}

    protected[gui] def translateToScreen(): AnyVal = 
    {
        val Point(sx, sy) = parent.convertPointToScreen(Point.zeroPoint)
        RenderSystem.translated(-sx, -sy, 0)
    }
    protected[gui] def translateFromScreen(): AnyVal = 
    {
        val Point(sx, sy) = parent.convertPointToScreen(Point.zeroPoint)
        RenderSystem.translated(sx, sy, 0)
    }
    ...
    /**
      ...
      */
    def frameUpdate_Impl(mouse:Point, rframe:Float): AnyVal = {}

    /** Called when this node is added to another as a child. */
    def onAddedToParent_Impl(): AnyVal = {}
    ...
    /**
      ...
      * @param rframe The partial frame until the next frame.
      */
    def drawBack_Impl(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = {}
    ...
    /**
      ...
      * @param rframe The partial frame until the next frame.
      */
    def drawFront_Impl(stack:MatrixStack, mouse:Point, rframe:Float): AnyVal = {}
}
