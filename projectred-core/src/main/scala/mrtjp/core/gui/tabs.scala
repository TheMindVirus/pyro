...
class TabNode(wMin:Int, hMin:Int, wMax:Int, hMax:Int, val color:Int) extends TNode
{
    ...
    def drawTab(stack:MatrixStack): AnyVal = {}

    def drawIcon(stack:MatrixStack): AnyVal = {}

    def buildToolTip(list:ListBuffer[String]): AnyVal = {}

    def drawBox(stack:MatrixStack): AnyVal = 
    {
...
trait TStackTab extends TabNode
{
    ...
    abstract override def drawIcon(stack:MatrixStack): AnyVal = 
    {
...
class TabControlNode(x:Int, y:Int) extends TNode
{
    ...
    /**
      ...
      */
    def onTabClicked(tab:TabNode): AnyVal = 
    {
        if (tab != active)
        {
    ...
    override def frameUpdate_Impl(mouse:Point, rframe:Float): AnyVal = 
    {
        var dy = 0
        for (w <- ourChildren)
        {
