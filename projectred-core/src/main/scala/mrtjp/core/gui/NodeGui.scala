...
//    def this(w:Int, h:Int) = this(new NodeContainer(null, -1), w, h, null, null)
    ...
    final override def init(): AnyVal = 
    {
        super.init()
    ...
    final override def tick(): AnyVal = 
    {
        super.tick()
        update()
    }

    final override def render(stack:MatrixStack, mouseX: Int, mouseY: Int, partialTicks: Float): AnyVal = 
    {
    ...
    final override def init(mc:Minecraft, i:Int, j:Int): AnyVal = {
...
//        keycode == 1 || keycode == minecraft.gameSettings.keyBindInventory.getKey //esc or inv key
    ...
    final override def renderBg(stack:MatrixStack, f:Float, mx:Int, my:Int): AnyVal = 
    {
        lastFrame = f
    ...
    final override def renderLabels(stack:MatrixStack, mx:Int, my:Int): AnyVal = 
    {
        val mouse = new Point(mx, my)
        RenderSystem.disableDepthTest()
        RenderSystem.color4f(1, 1, 1, 1)
