...
trait TTargetParticle extends CoreParticle
{
    ...
    def tx_=(x:Double): AnyVal = {target.x = x}
    def ty_=(y:Double): AnyVal = {target.y = y}
    def tz_=(z:Double): AnyVal = {target.z = z}

    def ptx = prevTarget.x
    def pty = prevTarget.y
    def ptz = prevTarget.z

    def ptx_=(x:Double): AnyVal = {prevTarget.x = x}
    def pty_=(y:Double): AnyVal = {prevTarget.y = y}
    def ptz_=(z:Double): AnyVal = {prevTarget.z = z}

    def dtx = tx-ptx
    def dty = ty-pty
    def dtz = tz-ptz

    def setTarget(x:Double, y:Double, z:Double): AnyVal = 
    {
        target.set(x, y, z)
    }

    abstract override def tick(): AnyVal = 
    {
        super.tick()
