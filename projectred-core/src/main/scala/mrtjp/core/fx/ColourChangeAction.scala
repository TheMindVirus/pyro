...
trait TColourParticle extends CoreParticle
{
    ...
    def red_=(r:Double): AnyVal = {rgb.x = r.toFloat}
    def green_=(g:Double): AnyVal = {rgb.y = g.toFloat}
    def blue_=(b:Double): AnyVal = {rgb.z = b.toFloat}

    def setRGB(r:Double, g:Double, b:Double): AnyVal = 
    {
        red = r
        green = g
        blue = b
