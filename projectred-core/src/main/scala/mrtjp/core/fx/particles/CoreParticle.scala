...
class CoreParticle(w:ClientWorld) extends Particle(w, 0.0D, 0.0D, 0.0D) {
    ...
    def setAge(age: Int): AnyVal = {
        this.age = age
    }

    def getAge = age

    def runAction(action: ParticleAction): AnyVal = {
    ...
    def removeAction(action: ParticleAction): AnyVal = {
