...
object Messenger
{
     ...
     * @param mail
     */
    def addMessage(x: Double, y: Double, z: Double, mail: String): AnyVal = 
    {
    ...
    @SubscribeEvent
    @OnlyIn(Dist.CLIENT)
    def renderMessages(event:RenderWorldLastEvent): AnyVal = 
    {
    ...
    @OnlyIn(Dist.CLIENT)
    private def readMessage(mStack:MatrixStack, m:Message, time:Double): AnyVal = 
    {
        var width = 0
        var height = 0
...
object Replace extends MailOption
{
    ...
    override def change(mes: Message): AnyVal = 
    {
