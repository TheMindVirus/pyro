...
object KeyTracking
{
    ...
    def updatePlayerKey(id:Int, player:PlayerEntity, state:Boolean): AnyVal
    {
        map(id) += player -> state
    }

    def registerTracker(tracker:TServerKeyTracker): AnyVal
    {
        tracker.id = idPool
        idPool += 1
...
trait TClientKeyTracker
{
    ...
    @SubscribeEvent
    @OnlyIn(Dist.CLIENT)
    def tick(event:ClientTickEvent): AnyVal = {
